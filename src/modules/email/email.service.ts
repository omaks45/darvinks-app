...
export class MailService {
  ...
  constructor(private readonly config: ConfigService<AppConfig>) {
    ...
    this.transporter = nodemailer.createTransport({
      ...
      secure: this.mailCfg.port === 465,
      auth: {
        user: this.mailCfg.user,
        pass: this.mailCfg.password.replace(/\s/g, ''),
      },
  ...
  async sendProvisioningEmail(payload: {
    to: string; fullName: string; roleLabel: string;
    employeeRef: string; temporaryPassword: string;
  }): Promise<void> {
    const { to, fullName, roleLabel, employeeRef, temporaryPassword } = payload;
    await this.send({
      to,
      subject: 'Your TB DARVINKS Account Has Been Created',
      html: this.provisioningHtml(fullName, roleLabel, employeeRef, temporaryPassword),
    });
    this.logger.log(`Provisioning email sent → ${to} (${employeeRef})`);
  }

  async sendPasswordResetEmail(payload: {
    to: string; fullName: string; temporaryPassword: string;
  }): Promise<void> {
    const { to, fullName, temporaryPassword } = payload;
    await this.send({
      to,
      subject: 'TB DARVINKS — Your Password Has Been Reset',
      html: this.passwordResetHtml(fullName, temporaryPassword),
    });
  ...
  async sendForgotPasswordEmail(payload: {
    ...
    await this.send({
      to,
      subject: 'TB DARVINKS — Password Reset OTP',
      html: this.forgotPasswordHtml(fullName, otp),
    });
  ...
  async sendInviteEmail(payload: {
    ...
    const { to, roleLabel, inviteUrl, expiresHours } = payload;
    await this.send({
      to,
      subject: `You've been invited to join TB DARVINKS`,
      html: this.inviteHtml(roleLabel, inviteUrl, expiresHours),
    });
    this.logger.log(`Invite email sent → ${to} (${roleLabel})`);
  }
  ...
  // ── HTML builders (no Python escaping — pure TS template literals) ──────────

  private header(): string {
    return `
      <table width="100%" cellpadding="0" cellspacing="0"
              style="background:linear-gradient(135deg,#5C0F18 0%,#8B1520 100%);padding:36px 40px;">
        <tr>
          <td align="center">
        ...
        <tr>
          <td style="padding:24px 40px;text-align:center;">
            <p style="margin:0 0 4px;font-size:12px;color:#888;">
              Darvinks Healthcare Ltd &nbsp;&middot;&nbsp;
...
<body style="margin:0;padding:0;background:#f4f4f4;font-family:'Helvetica Neue',Arial,sans-serif;">
  ...
  private provisioningHtml(
    fullName: string, roleLabel: string,
    employeeRef: string, temporaryPassword: string,
  ): string {
    const body = `
      <tr><td style="padding:40px 40px 24px;">
        ...
        <table width="100%" cellpadding="0" cellspacing="0"
                style="background:#fdf6f7;border:1px solid #f0d0d4;border-radius:8px;margin-bottom:24px;">
          <tr><td style="padding:24px;">
            <p style="margin:0 0 4px;font-size:11px;font-weight:700;letter-spacing:0.15em;
                      color:#8B1520;text-transform:uppercase;">Your Login Credentials</p>
            <table width="100%" cellpadding="0" cellspacing="0" style="margin-top:16px;">
              <tr>
                <td style="padding:8px 0;border-bottom:1px solid #f0d0d4;font-size:13px;color:#888;width:140px;">
                  Employee Ref</td>
                <td style="padding:8px 0;border-bottom:1px solid #f0d0d4;font-size:14px;font-weight:700;color:#1a1a1a;">
                  ${employeeRef}</td>
              </tr>
              <tr>
                <td style="padding:8px 0;font-size:13px;color:#888;">Temporary Password</td>
                <td style="padding:8px 0;font-size:14px;font-weight:700;color:#1a1a1a;font-family:monospace;">
  ...
  private passwordResetHtml(fullName: string, temporaryPassword: string): string {
    const body = `
      <tr><td style="padding:40px 40px 24px;">
        <h2 style="margin:0 0 8px;font-size:22px;font-weight:700;color:#1a1a1a;">
          Password Reset
        ...
        <p style="margin:0 0 24px;font-size:15px;color:#555;line-height:1.6;">
          Hi ${fullName}, your TB DARVINKS account password has been reset by an administrator.
        </p>
        <table width="100%" cellpadding="0" cellspacing="0"
                style="background:#fdf6f7;border:1px solid #f0d0d4;border-radius:8px;margin-bottom:24px;">
          <tr><td style="padding:24px;">
            <p style="margin:0 0 12px;font-size:11px;font-weight:700;letter-spacing:0.15em;
                      color:#8B1520;text-transform:uppercase;">New Temporary Password</p>
            <p style="margin:0;font-size:22px;font-weight:700;color:#1a1a1a;
                      font-family:monospace;letter-spacing:0.1em;">${temporaryPassword}</p>
          </td></tr>
        </table>
        <table width="100%" cellpadding="0" cellspacing="0"
  ...
  private forgotPasswordHtml(fullName: string, otp: string): string {
    const body = `
      <tr><td style="padding:40px 40px 24px;">
        <h2 style="margin:0 0 8px;font-size:22px;font-weight:700;color:#1a1a1a;">
          Reset Your Password
        ...
        <p style="margin:0 0 24px;font-size:15px;color:#555;line-height:1.6;">
          Hi ${fullName}, we received a request to reset your TB DARVINKS password.
          Use the OTP below to proceed.
        </p>
        <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
          <tr>
            <td align="center"
                style="background:#fdf6f7;border:2px dashed #8B1520;border-radius:12px;padding:32px 40px;">
              <p style="margin:0 0 8px;font-size:11px;font-weight:700;letter-spacing:0.2em;
                        color:#8B1520;text-transform:uppercase;">Your One-Time Password</p>
              <p style="margin:0;font-size:42px;font-weight:700;letter-spacing:0.25em;
                        color:#1a1a1a;font-family:monospace;">${otp}</p>
  ...
  private inviteHtml(roleLabel: string, inviteUrl: string, expiresHours: number): string {
    const body = `
      <tr><td style="padding:40px 40px 24px;">
        <h2 style="margin:0 0 8px;font-size:22px;font-weight:700;color:#1a1a1a;">
          You've Been Invited!
        ...
        <p style="margin:0 0 24px;font-size:15px;color:#555;line-height:1.6;">
          You have been invited to join <strong>TB DARVINKS</strong> as a
          <strong>${roleLabel}</strong>. Click the button below to complete
          your registration.
        </p>
        <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
          <tr>
            <td align="center">
              <a href="${inviteUrl}"
                  style="display:inline-block;background:#8B1520;color:#ffffff;
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
          <tr>
            <td style="background:#f4f4f4;border-radius:6px;padding:12px 16px;
                        font-size:12px;color:#555;font-family:monospace;word-break:break-all;">
              ${inviteUrl}
            </td>
          </tr>
        </table>
        <table width="100%" cellpadding="0" cellspacing="0"
          ...
          <tr><td style="padding:16px 20px;">
            <p style="margin:0;font-size:13px;color:#92400e;line-height:1.6;">
              ...
              It can only be used once and only by the email address it was sent to.
            </p>
          </td></tr>
        </table>
      </td></tr>`;
